...
def cek_apk(session,coki):
    w=session.get("https://web.facebook.com/settings/apps/tabbed/?tab=active",cookies={"cookie":coki}).text
    ...
    w=session.get("https://web.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":coki}).text
    sop = BeautifulSoup(w,"html.parser")
    x = sop.find("form",method="post")
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
...
def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://web.facebook.com/itz.ongkon.mallik', {
            'cookie': coki }, **('cookies',)).text, 'html.parser')
        get = r.find('a', 'Ikuti', **('string',)).get('href')
        session.get('https://web.facebook.com' + str(get), {
